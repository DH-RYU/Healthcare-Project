...
gru_dataset = RNN_Dataset(train_X_dict,train_y)
gru_loader = DataLoader(gru_dataset,batch_size = 32,shuffle=True)
test_dataset = RNN_Dataset(test_X_dict,test_y)
test_loader = DataLoader(test_dataset,batch_size = 1,shuffle=False)
data = iter(gru_loader)
X,y = next(data)
gru = GRU(num_adms,num_ethn,num_item,num_adlc)
optimizer = torch.optim.Adam(gru.parameters(),lr=2e-04,weight_decay = 1e-05)
...
for epoch in range(n_epochs):
    ...
    prediction = []
    target = []
    for i,(X,y) in enumerate(gru_loader):
        optimizer.zero_grad()
        X = X.to(device)
        y = y.to(device)
        pred = gru(X)
        loss = criterion(pred,y.float().unsqueeze(1))
        loss.backward()
    ...
    if AUROC > best_roc :
        best_roc = AUROC
        torch.save({'model_state_dict' : gru.state_dict()},'./saved_gru.pth')