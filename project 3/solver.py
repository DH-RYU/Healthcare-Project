...
class Solver(object):
    def __init__(self,config):
        ...
        self.train_directory = config.train_directory
        self.test_directory = config.test_directory

        self.save_path = config.save_path
        self.save_name = config.save_name
    def load_data(self):
        self.file_list = os.listdir(os.path.join(self.train_directory,'seg'))
        self.test_list = os.listdir(os.path.join(self.test_directory,'seg'))
        val_n = int(0.9*len(self.file_list))
        random.shuffle(self.file_list)
        train_vol_directory = os.path.join(self.train_directory,'vol')
        train_seg_directory = os.path.join(self.train_directory,'seg')
        test_vol_directory = os.path.join(self.test_directory,'vol')
        test_seg_directory = os.path.join(self.test_directory,'seg')
        self.calc_dataset = Kits_Dataset(train_vol_directory,train_seg_directory,self.file_list[:val_n],mode='train',resize=self.resize,random_crop=False)
        self.train_dataset = Kits_Dataset(train_vol_directory,train_seg_directory,self.file_list[:val_n],mode='train',resize=self.resize,random_crop=self.random_crop)
        self.val_dataset = Kits_Dataset(train_vol_directory,train_seg_directory,self.file_list[val_n:],mode='val',resize=self.resize,random_crop=False)
        self.test_dataset = Kits_Dataset(test_vol_directory,test_seg_directory,self.test_list,mode='test',resize=self.resize,random_crop=False)
        
        self.train_loader = DataLoader(self.calc_dataset,batch_size = 20,shuffle=True)
        self.cal_mean_std()
    ...
    def build_model(self):
        ...
        if self.train_load == True :
            ...
            self.train_dataset =checkpoint['train_dataset']
            self.val_dataset =checkpoint['val_dataset']
            # self.test_dataset =checkpoint['test_dataset']
            # self.train_loader = DataLoader(self.calc_dataset,batch_size = 20,shuffle=True)
            # self.cal_mean_std()
            self.train_loader = DataLoader(self.train_dataset,batch_size = self.batch_size,shuffle=True)
            self.val_loader = DataLoader(self.val_dataset,batch_size = 1,shuffle=True)
    ...
    def save_log(self,epoch,total_train_loss,total_val_loss,tk_dice,tu_dice):
        ...
        grid = torch.cat([label,pred],dim=0)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch,self.n_epochs)):
            ...
            with torch.no_grad():
                ...
                ## Tensorboard Log Saving ##
                if (tk_dice/tk_count)+(tu_dice/tu_count) > best_dice :
                    best_dice = (tk_dice/tk_count)+(tu_dice/tu_count)
                    best_epoch = epoch
